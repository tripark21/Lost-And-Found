...
$order_map = ['newest'=>'date_reported DESC, created_at DESC','oldest'=>'date_reported ASC, created_at ASC','name_az'=>'item_name ASC','name_za'=>'item_name DESC'];
$order = $order_map[$sort] ?? $order_map['newest'];

$where  = "WHERE item_type='found' AND status='unresolved'";
$params = [];
if ($search!==''){$where.=" AND (item_name ILIKE :s OR description ILIKE :s OR location ILIKE :s)";$params[':s']='%'.$search.'%';}
if ($category!==''){$where.=" AND category=:cat";$params[':cat']=$category;}

$cnt=$conn->prepare("SELECT COUNT(*) FROM items $where");$cnt->execute($params);$total=(int)$cnt->fetchColumn();
...
$stmt->bindValue(':lim',$per_page,PDO::PARAM_INT);$stmt->bindValue(':off',$offset,PDO::PARAM_INT);
foreach($params as $k=>$v)$stmt->bindValue($k,$v);$stmt->execute();$items=$stmt->fetchAll();

$cats=$conn->query("SELECT DISTINCT category FROM items WHERE item_type='found' ORDER BY category")->fetchAll();

function q(array $ov):string{$b=['search'=>$_GET['search']??'','category'=>$_GET['category']??'','sort'=>$_GET['sort']??'newest','page'=>$_GET['page']??1];return '?'.http_build_query(array_filter(array_merge($b,$ov),fn($v)=>$v!==''&&$v!==0&&$v!=='0'));}
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<nav class="navbar">
    <div class="nav-container">
        ...
        <ul class="nav-menu">
            ...
            <li><a href="found_items_postgres.php" class="active">Found Items</a></li>
...
<div class="container">
    <div class="page-header">
        <h1>Found Items</h1>
        <p>Items found and turned in on campus. Recognize something? Contact the finder directly.</p>
    </div>
    ...
    <div class="filters-bar">
        <form method="GET" class="filters-row">
            ...
            <div class="filter-actions">
                ...
                <a href="found_items_postgres.php" class="btn btn-ghost">Reset</a>
            </div>
        </form>
    ...
    <div class="items-grid">
        ...
        <?php else:?>
            <div class="empty-state">
                ...
                <p><?php echo($search||$category)?'Try adjusting your filters.':'No found items reported yet.';?></p>
                <a href="../report_found_postgres.php" class="btn btn-accent">Report a Found Item</a>
            </div>
        <?php endif;?>
    </div>
